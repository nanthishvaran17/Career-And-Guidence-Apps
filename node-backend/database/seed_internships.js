...
const seedData = () => {
    ...
    db.serialize(() => {
        ...
        const insertCollege = db.prepare("INSERT INTO colleges (name, location, field, fees, rating, type, website, details_link, accommodation) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
        colleges.forEach(college => insertCollege.run(college.name, college.location, college.field, college.fees, college.rating, college.type, college.website, college.details_link, college.accommodation));
        ...
        const stmtIntern = db.prepare("INSERT INTO internships (title, company, type, stipend, duration, requirements, link) VALUES (?, ?, ?, ?, ?, ?, ?)");
        internships.forEach(i => {
            stmtIntern.run(i.title, i.company, i.type, i.stipend, i.duration, i.requirements, i.link || "https://linkedin.com/jobs");
        });
        stmtIntern.finalize();
