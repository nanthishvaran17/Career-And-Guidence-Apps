...
export function EducationGuidance() {
  ...
  // URL Parameter & Context-Aware Tab Initializer
  useEffect(() => {
    ...
    if (tabParam) {
      if (['schools', 'colleges', 'scholarships'].includes(tabParam)) {
    ...
    if (profileStr && !tabParam) { // Only fallback to profile logic if no tab param is provided
      try {
        const profile = JSON.parse(profileStr);
        const level = (profile.educationLevel || profile.education_level || '').toLowerCase();
        
        if (level.includes('ug') || level.includes('college') || level.includes('undergraduate') || level.includes('degree')) {
          setActiveTab('colleges');
        } else {
          setActiveTab('schools');
        }
      } catch (e) {
        console.error("Error parsing profile for education guidance", e);
      }
    }
  }, []);
                          ...
                          <div className="flex justify-between items-start gap-3">
                            <div className="flex-1 min-w-0">
                              <h3 
                                ...
                                className="font-bold cursor-pointer text-base text-gray-900 leading-tight group-hover:text-indigo-600 transition-colors line-clamp-2 hover:underline">
                                {college.name}
                              </h3>
